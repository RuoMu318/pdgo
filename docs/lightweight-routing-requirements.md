...
> 状态：三模式提示词／配置纵切已形成源码候选并通过本地自动与静态验证；尚未全局安装，也尚未运行付费、Token、耗时或质量基准。
>
...
本轮源码候选只修改 Codex-native 启动提示、秘书加载边界、兼容配置、合同测试和说明文档；不修改 dispatcher、schema、manifest、安装器、依赖、全局安装或生产配置，也不授权外部动作。
...
4. 通过第 6 节四方基准后，才把任何拟议成本或质量门槛改为实测结论；
