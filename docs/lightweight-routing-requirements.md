...
> 状态：v4 本地候选把秘书拆成常驻前台和按风险加载的完整治理；v3 有界本地动作路由已通过本地验证并安装到本机 Codex，但本轮源码尚未重新安装，宿主也未提供可信的自动分流证明或账单 Token。
>
> 原则：先按任务的风险、可逆性和验收需要分流，再加载完成任务所需的最小流程。未实测阈值只能称为“拟议门槛”，不得宣称已经省钱或通过验证。

## 1. 目标与边界

本需求把任务分为轻量模式、标准模式和高保障模式，使治理成本与任务风险相称，同时让当前 Agent 始终以秘书前台承接用户、说明实质变化并收口。前台是表达与责任入口，不等于加载秘书 Skill 或完整治理。
...
| 模式 | 定义 | 典型例子 |
| --- | --- | --- |
| 轻量模式 | 当前主 Agent 以秘书前台承接并直接完成明确、局部、低风险、可撤销的工作，不产生 PDGO 治理开销 | 简单问答、翻译、短改写、指定材料的只读解释、用户明确要求的本地小改 |
| 标准模式 | 当前主 Agent 继续作为秘书前台；任务可在当前工作区内完成和回滚，但复杂度、文件数量或验证范围已超过轻量边界，采用有界执行和相称自检 | 普通功能修改、边界固定且可测试的多文件机械修改、局部故障修复 |
...
秘书前台在有实质内容的接单、边界或风险变化、最终收口时出现；不要求逐句加角色标签，也不默认向用户讲模式术语。该前台来自常驻启动规则，不计作秘书治理 Skill、治理提示或角色流程加载。

### 3.2 标准模式

满足以下全部条件，但超过轻量规模时，进入标准模式。秘书前台仍由当前 Agent 承担，标准模式不加载秘书治理 Skill 或治理提示，也不新增 PDGO 批准：
...
- **Given** 用户发来任何有实质内容的任务；
- **When** 当前 Agent 接单、遇到边界或风险实质变化、或完成收口；
- **Then** 由同一个当前 Agent 以大白话承担秘书前台，不要求用户显式喊“秘书”，也不把前台表现计作治理提示、额外模型调用、子 Agent、状态写入或新增批准。
...
9. 证明秘书前台在三种模式中始终存在，同时轻量和标准模式仍不加载秘书治理 Skill、治理提示、正式计划、PDGO 状态或角色流程。
