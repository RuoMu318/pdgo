...
> 状态：v1 提示词／配置纵切已通过本地验证；v2 可执行路由和授权包络仍是未安装的源码候选，只有完整验证与独立审核后才能称为已接受。
>
...
本轮源码候选在 v1 启动提示基础上增加 dispatcher 纯路由器、可选授权策略、五个消息 schema、合同测试和说明文档；不修改 CLI wrapper、manifest、安装器、依赖、全局安装或生产配置，也不授权外部动作。
...
唯一允许进入标准模式的临时权限例外必须同时满足：精确一个应用、本机范围、仅当前会话、可撤销、来自用户当次明确请求，并且 `administrator`、`account`、`network`、`secrets`、`wildcard`、`third_party`、`long_lived`、`irreversible` 八项风险全部明确为 `false`。任一字段缺失、应用数不是一、范围扩大或任一风险为真，都在工作量判断前进入高保障模式。该例外只减少 PDGO 治理深度，不取消宿主或操作系统仍要求的权限提示。

### 3.3 高保障模式

出现任一条件即进入或升级高保障模式：

- 对外发送、提交、发布、部署、购买、支付，或不满足第 3.2 节唯一例外的权限变更；
...
可执行分类器必须先检查高保障风险和临时权限完整性，再读取 `workload`。风险证明或工作量值缺失时一律 fail closed（停止并进入高保障），不能用默认 standard 掩盖未知。
...
宿主证明只能由注入的可信 transport／adapter 返回。批准 JSON、父 Agent 文本或消息中自报的 `verified: true` 都不是证明。启用策略后，同一 `authorization_envelope` 必须贯穿 plan、approval、dispatch、execution report 和 review decision；缺失、不一致、过期、plan/version 漂移、范围漂移或角色漂移都 fail closed。未启用策略的旧计划保持兼容。
